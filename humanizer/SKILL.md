---
name: humanizer
description: Strip AI-writing fingerprints from prose and give it real voice. Applies 34 documented patterns from Wikipedia's "Signs of AI writing" guide — significance inflation, rule-of-three lists, copula avoidance, em dash overuse, rhetorical questions, reassurance kickers, and more. Use when writing or editing any human-facing prose: website copy, blog posts, marketing pages, metadata and schema descriptions, emails, docs, PR descriptions, release notes. Also triggers on "humanize", "de-AI", "de-slop", "un-ChatGPT", "make this sound human", "does this read as AI written".
license: MIT (Siqi Chen / @blader — see LICENSE)
---
...
Also apply this skill to **your own** output when writing user-facing prose such as release notes, PR descriptions, docs, and summaries. A careful baseline voice already strips most of these, but a focused pass catches what slips through.
...
2. **File.** The user points at a file. Use `Read` to load it, then `Edit` to apply targeted changes or `Write` for a full rewrite. For a page or doc in a repo, a targeted `Edit` per section is cleaner than rewriting the whole file.
...
Always show the user what changed. For file edits, describe the changed sections in plain language rather than silently overwriting.

Apply this skill to **your own** user-facing prose too: docs, release notes, PR descriptions, commit bodies, and summaries.
